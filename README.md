...
| **senior_researcher** | 高级研究员 | 撰写完整研究报告 | 融合全部 Agent 产出；首稿 8192 token + 分章预算制扩写保障万字篇幅；LLM 无法复写 base64，图表由代码程序化追加"附录：关键数据图表" |
...
- 首稿放开至 8192 token（DeepSeek 单次输出上限），提示词要求每章 ≥1200 字、正文 ≥10000 汉字
- 单次调用不达标时按章节**预算制扩写**（`_ensure_length`）：剩余缺口逐节分配、每节净增封顶 900 字、总预算到顶即停、最多 2 轮；扩写只允许引用章节内与材料中已有数据，防扩写引入幻觉
- 目标字数可通过 `REPORT_TARGET_CJK` 环境变量配置；实测三例：一把过 10936 字、9842→10591 字（目标 1 万）、5233→13299 字（目标 1.3 万，budget 余量 1）
...
| **报告长度** | **≥10000 汉字** | 首稿 8192 token + 分章预算制扩写（P97/P99/P100 实测 10936/13299/10591 字），图表附录另计；目标可配 `REPORT_TARGET_CJK`；扩写使 auto 全流程增至 240-270s |
